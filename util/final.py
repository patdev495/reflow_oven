...
# CẤU HÌNH
# =======================
source_dir = r"C:\Users\AKTF\Downloads\GỬI ANH ĐÀ EXCEL PROFILE\GỬI ANH ĐÀ EXCEL PROFILE"
output_file = r"summary11.csv"
...
def smart_read_csv_lines(path):
    """Đọc CSV linh hoạt: detect encoding + split thủ công"""
...
def get_parent_folders(csv_path, base_dir):
    """
    Trả về chuỗi thư mục cha:
    FolderA / FolderB / FolderC
    """
    rel_path = os.path.relpath(os.path.dirname(csv_path), base_dir)
    if rel_path == ".":
        return ""
...
# Duyệt đệ quy CSV
csv_files = sorted(
    glob.glob(os.path.join(source_dir, "**", "*.csv"), recursive=True)
)
...
for csv_path in csv_files:
    ...
    result = {
        ...
        "Parent Folders": get_parent_folders(csv_path, source_dir)
    }
    ...
    for i in range(1, 13):
        ...
        if not pos:
            pos = find_first_containing(lines, f"Area {i}")

        result[f"zone {i}"] = get_zone_value(lines, *pos) if pos else ""
...
if records:
    ...
    cols_order = (
        ["File Name", "Parent Folders"]
        + keywords_first
        + [f"zone {i}" for i in range(1, 13)]
        + [keyword_pcc, keyword_space]
    )
